...
namespace CoreMVC2.Controllers
{
    ...
    public class APIController : ControllerBase
    {
        ...
        public IActionResult GetOverallByBetType([FromBody] OverallByBetTypeInput model)
        {
            ...
            sql = sql + "drop table if exists #TempReport  ";
            sql = sql + "create table #TempReport (  ";
            sql = sql + "[ID] int null, [BET_TYPE] Nvarchar(200) null, [LotteryTypeID] int null, [DrawTypeID] int null, [UpdateDate] datetime null,  ";
            sql = sql + "[Username] Nvarchar(200) null, [Bill_No_Ticket] Nvarchar(200) null, [TOVER] Numeric (32, 4) null,  ";
            sql = sql + "[Pending] numeric(32, 4) null, [Openning_Time] datetime null, [BetTime] datetime null, [Bet_1] Nvarchar(200) null,  ";
            sql = sql + "[Bet_2] Nvarchar(200) null, [Bet_3] Nvarchar(200) null, [BetAmount] Numeric (32, 4) null, [WinMoney] Numeric (32, 4) null,  ";
            sql = sql + "[IsWin] int null, Sum_winlose_AllLOST numeric (32, 4) null, Sum_winlose_AllWIN_4d numeric (32, 4) null,  ";
            sql = sql + "Sum_winlose_AllWIN_NOT4d numeric (32, 4) null, [WL] Numeric (32, 4) null, [Lost] numeric (32, 4) null,  ";
            sql = sql + "[Win] numeric (32, 4) null ) insert into #TempReport select a.ID, c.LotteryTypeName as BET_TYPE, b.LotteryTypeID,  ";
            sql = sql + "DrawTypeID, a.UpdateDate, a.Username, a.CurrentPeriod as Bill_No_Ticket, isnull(Price, 0) as TOVER,  ";
            sql = sql + "( case when (iswin is null) then TRY_CONVERT(numeric(38,12),Price) else 0 end ) as Pending,  ";
            sql = sql + "ShowResultDate as Openning_Time, a.CreateDate as BetTime, FamliyBigID as Bet_1, b.LotteryInfoName as Bet_2,  ";
            sql = sql + "SelectedNums as Bet_3, isnull(DiscountPrice, 0) as BetAmount, isnull(WinMoney, 0), IsWin,  ";
            sql = sql + "( case when ( a.UpdateDate between @Date11 AND @Date22 and WinMoney = '0.0000' and Iswin is not null ) then ISNULL(TRY_CONVERT(numeric(38,12),WinMoney)-TRY_CONVERT(numeric(38,12),DiscountPrice), 0) else 0 end ) as Sum_winlose_AllLOST,  ";
            sql = sql + "( case	when (a.UpdateDate between @Date11 AND @Date22 and DrawTypeID between 142 and 152 and WinMoney <> '0.0000' and Iswin is not null) then ISNULL(TRY_CONVERT(numeric(38,12),WinMoney)-TRY_CONVERT(numeric(38,12),DiscountPrice), 0) else 0 end ) as Sum_winlose_AllWIN_4d,  ";
            sql = sql + "( case	when (a.UpdateDate between @Date11 AND @Date22 and DrawTypeID NOT between 142 and 152 and WinMoney <> '0.0000' and Iswin is not null) then isnull( TRY_CONVERT(numeric(38,12),WinMoney), 0) else 0 end ) as Sum_winlose_AllWIN_NOT4d,  ";
            sql = sql + "0.00 as WL, 0.00 as Lost, 0.00 as Win from [dbo].[MPlayer] a inner join [dbo].[LotteryInfo] b on a.LotteryInfoID = b.LotteryInfoID  ";
            sql = sql + "inner join [dbo].[LotteryType] c on b.LotteryTypeID = c.LotteryTypeID  ";
            sql = sql + "inner join [dbo].GameDealerMemberShip d on a.GameDealerMemberID = d.MemberID  ";
            ...
            //sql = sql + "where a.UpdateDate between @Date11 AND @Date22 and c.LotteryTypeName = '@dbBetType'  ";
            ...
            sql = sql + "Bet_Type nvarchar(max) null, DrawTypeID int null, UserName nvarchar(max) null,  ";
            sql = sql + "Bill_No_Ticket nvarchar(max) null, Openning_Time datetime null, [UpdateDate] datetime null, ";
            sql = sql + "IsWin int null, TOver numeric (32, 12) null, Pending numeric (32, 12) null,  ";
            sql = sql + "BetAmount numeric (32, 12) null, WL numeric (32, 12) null, Sum_AllLost numeric (32, 12) null,  ";
            sql = sql + "Sum_AllWin_4d numeric (32, 12) null, Sum_AllWin_Not4D numeric (32, 12) null,  ";
            sql = sql + "TotalWin numeric (32, 12) null, TotalLost numeric (32, 12) null, Margin numeric (32, 12) null )  ";
            sql = sql + "insert into #TempSummary  ";
            sql = sql + "select Bet_Type , DrawTypeID , UserName , Bill_No_Ticket , Openning_Time, [UpdateDate] , IsWin  ";
            sql = sql + ", sum(TOver) as TOver , sum(Pending) as Pending , sum(BetAmount) as BetAmount , sum(WL) as MemberWinLose  ";
            sql = sql + ", sum(Sum_winlose_AllLOST) as Sum_AllLOST , sum(Sum_winlose_AllWIN_4d) as Sum_AllWIN_4d  ";
            sql = sql + ", sum(Sum_winlose_AllWIN_NOT4d) as Sum_AllWIN_NOT4d ,  ";
            sql = sql + "TotalWin = case when (#TempReport.isWin = 1) then sum(Sum_winlose_AllWIN_4d) + sum(Sum_winlose_AllWIN_NOT4d) + sum(BetAmount) else 0 end ,  ";
            sql = sql + "TotalLost = sum(Sum_winlose_AllLOST)  , Margin =  sum(BetAmount) - sum(Sum_winlose_AllLOST)    ";
            sql = sql + "from #TempReport  ";
            sql = sql + "group by Bet_Type , DrawTypeID , UserName , Bill_No_Ticket , Openning_Time, [UpdateDate] , IsWin  ";
            sql = sql + "order by UserName, Bill_No_Ticket  ";
            ...
            sql = sql + "select Bet_Type , DrawTypeID , UserName , Bill_No_Ticket , Openning_Time, [UpdateDate] , sum(TOver) as TOver , sum(Pending) as Pending  ";
            sql = sql + ", sum(BetAmount) as BetAmount , sum(WL) as MemberWinLose , sum(Sum_AllLOST) as Sum_AllLOST , sum(Sum_AllWIN_4d) as Sum_AllWIN_4d  ";
            sql = sql + ", sum(Sum_AllWIN_NOT4d) as Sum_AllWIN_NOT4d ,  ";
            sql = sql + "case  ";
            sql = sql + "when (DrawTypeID NOT between 142 and 152) then sum(TotalWin) - sum(Margin) else sum(TotalWin)  ";
            sql = sql + "end as TotalWin  ";
            sql = sql + ", sum(TotalLost) as TotalLost , sum(Margin) as TotalMargin  ";
            sql = sql + "from #TempSummary  ";
            sql = sql + "group by Bet_Type , DrawTypeID , UserName , Bill_No_Ticket, [UpdateDate] , Openning_Time  ";
            sql = sql + "order by Bill_No_Ticket desc  ";
            sql = sql + "select UserName  ";
            sql = sql + ", Bill_No_Ticket ";
            sql = sql + ", Openning_Time ";
            sql = sql + ", [UpdateDate] ";
            sql = sql + ", sum(TOver) as TOver , sum(TotalWin) as TotalWin , sum(TotalLost) as TotalLost , sum(Pending) as TotalPending  ";
            sql = sql + "from #TempSummary2  ";
            sql = sql + "group by UserName, Bill_No_Ticket , Openning_Time, [UpdateDate] ";
            sql = sql + "order by UserName, Bill_No_Ticket , Openning_Time  ";
